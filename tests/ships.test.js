...
function testShip(shipType, shipName, shipShortName, shipCost, shipMaintenance) {
    let ship;
...
testShip(ColonyShip, 'Colony Ship', 'CO', 8, 0);
testShip(ShipYard, 'Ship Yard', 'SY', 6, 0);
testShip(MiningShip, 'Mining Ship', 'Miner', 5, 0);
testShip(Decoy, 'Decoy', 'Decoy', 1, 0);
testShip(Scout, 'Scout', 'SC', 6, 1);
testShip(Destroyer, 'Destroyer', 'DD', 9, 1);
testShip(Base, 'Base', 'Base', 12, 0);
testShip(Cruiser, 'Cruiser', 'CA', 12, 2);
testShip(Battlecruiser, 'Battlecruiser', 'BC', 15, 2);
testShip(Battleship, 'Battleship', 'BB', 20, 3);
testShip(Dreadnought, 'Dreadnought', 'DN', 24, 3);
testShip(MSPipeline, 'MS Pipeline', 'MS', 3, 0);
